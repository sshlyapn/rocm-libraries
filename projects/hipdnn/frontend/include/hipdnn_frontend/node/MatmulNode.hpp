...
public:
    ...
    Error pre_validate_node() const override
    {
        ...
        HIPDNN_RETURN_IF_FALSE(attributes.get_a(),
                               ErrorCode::ATTRIBUTE_NOT_SET,
                               "MatmulNode missing A input for pre-validation");

        HIPDNN_RETURN_IF_FALSE(attributes.get_b(),
                               ErrorCode::ATTRIBUTE_NOT_SET,
                               "MatmulNode missing B input for pre-validation");

        HIPDNN_RETURN_IF_FALSE(attributes.get_c(),
                               ErrorCode::ATTRIBUTE_NOT_SET,
                               "MatmulNode missing C output for pre-validation");
        ...
        HIPDNN_CHECK_ERROR(
            validateMinimumTensorDimensions(a, minRank, "Input tensor A"));
        HIPDNN_CHECK_ERROR(
            validateMinimumTensorDimensions(b, minRank, "Input tensor B"));
    ...
    Error infer_properties_node() override
    {
        ...
        HIPDNN_RETURN_IF_FALSE(a,
                               ErrorCode::ATTRIBUTE_NOT_SET,
                               "MatmulNode missing A input for setting properties");

        HIPDNN_RETURN_IF_FALSE(b,
                               ErrorCode::ATTRIBUTE_NOT_SET,
                               "MatmulNode missing B input for setting properties");

        HIPDNN_RETURN_IF_FALSE(c,
                               ErrorCode::ATTRIBUTE_NOT_SET,
                               "MatmulNode missing C output for setting properties");
        ...
        if(c->get_dim().empty())
        {
            ...
            for(size_t i = 0; i < batchDims; ++i)
            {
                ...
                HIPDNN_RETURN_IF_TRUE(aDimVal % bDimVal != 0 && bDimVal % aDimVal != 0,
                                      ...
                                      std::string("Matmul input tensors A and B have incompatible ")
                                          + "batch dimensions for broadcasting at index "
                                          + std::to_string(i) + ": A has dim="
                                          + std::to_string(aDimVal)
                                          + ", B has dim=" + std::to_string(bDimVal));
        ...
        {
            auto cStrides = hipdnn_sdk::utilities::generateStrides(c->get_dim());
        ...
        pack_node(flatbuffers::FlatBufferBuilder& builder) const override
    {
        return hipdnn_sdk::data_objects::CreateNodeDirect(
            ...
            hipdnn_sdk::data_objects::NodeAttributes::MatmulAttributes,
